class Kurz:
    ...
    def stav(self):
        if self.je_plny():
            return "Kurz je plný."
        return f"Zbývá {self.volna_mista()} volných míst."
    ...
    def vypis_strucne(self):
        print("Název kurzu: ", self.nazev)
        print("Počet studentů: ", len(self.studenti))

    def vypis(self, jen_neplnoleti=False, zobraz_vek=True):
        print("Název kurzu", self.nazev)
        ...
        for s in self.studenti:
            if jen_neplnoleti and s.vek() >= 18:
                continue
            if zobraz_vek:
                print("-", s)
            else:
                print(f"- {s.cele_jmeno()} ({s.email})")
